...
class Track:
    ...
    def _parse_fit_session_message(self, message):
        if 'total_elapsed_time' in message.get_values():
            self._elapsed_time = datetime.timedelta(seconds=message.get_values()['total_elapsed_time'])
        if 'total_timer_time' in message.get_values():
            self._timer_time = datetime.timedelta(seconds=message.get_values()['total_timer_time'])
            ...
            if t.type is not None:
                if track_type is not None:
                    if t.type != track_type:
                        print(f'{file_name}: tracks with differing types: "{track_type}" and "{t.type}"')
                else:
                    track_type = t.type
            for s in t.segments:
                segment = []
    ...
    def save_to_cache(self, cache_file_name: str):
        ...
        with open(cache_file_name, 'w') as file:
            ...
            if self._elapsed_time is not None:
                header.append(('elapsed_time', f'{self._elapsed_time.total_seconds():.1f}'))
