...
class TrackPoint:
    ...
    def serialize(self, prev: 'TrackPoint') -> str:
        if prev is None:
            ...
            lat = int(self._lat_lng.lat().degrees * 1000000.0) - int(prev._lat_lng.lat().degrees * 1000000.0)
            lng = int(self._lat_lng.lng().degrees * 1000000.0) - int(prev._lat_lng.lng().degrees * 1000000.0)
            altitude = int(self._altitude) - int(prev._altitude) if self._altitude is not None else None
        if altitude is None:
            return f'{time},{lat},{lng}'
        return f'{time},{lat},{lng},{altitude}'

    @staticmethod
    def deserialize(s: str, prev: 'TrackPoint') -> 'TrackPoint':
        tokens = s.split(',')
        ...
        if prev is None:
            ...
            altitude = (float(tokens[3]) + prev._altitude) if len(tokens) == 4 else None
